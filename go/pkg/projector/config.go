...
func getArgs(opts *Opts) ([]string, error) {
	...
	if operation == Add {
		if len(opts.Args) != 3 {
			return nil, fmt.Errorf("add Expected 2  args, got %v", len(opts.Args))
		}
	...
	if operation == Remove {
		if len(opts.Args) != 2 {
			return nil, fmt.Errorf("remove Expected 1 arg, got %v", len(opts.Args))
		}
	...
	if len(opts.Args) > 1 {
		return nil, fmt.Errorf("Expected 0 or 1 arg, got %v", len(opts.Args))
	}
